...
class FFprobeVideoContainer:
    ...
    def extract_subtitles(
        ...
        for subtitle in subtitles:
            ...
            sub_path = (
                f"{os.path.splitext(self.path)[0]}.{subtitle.language}-{subtitle.disposition}.{extension_to_use}"
            )
            if custom_dir is not None:
    ...
    def copy_subtitles(
        ...
        for subtitle in subtitles:
            sub_path = f"{os.path.splitext(self.path)[0]}.{subtitle.language}-{subtitle.disposition}.{subtitle.extension}"
            if custom_dir is not None:
                basename_callback = basename_callback or os.path.basename
                sub_path = os.path.join(custom_dir, basename_callback(sub_path))
